...
public unsafe class ResourceBarPercentages : GameModification {
    public override ModificationInfo ModificationInfo => new() {
        DisplayName = Strings.ResourceBarPercentages,
        Description = Strings.ResourceBarPercentagesDescription,
    ...
    public override void OnEnable() {
        ...
        configWindow = new ConfigAddon {
            ...
            Title = Strings.ResourceBarPercentagesConfig,
            Config = config,
        };

        configWindow.AddCategory(Strings.PartyList)
            .AddCheckbox(Strings.ShowOnPartyList, nameof(config.PartyListEnabled))
            .AddIndent()
            .AddCheckbox(Strings.ApplyToPlayer, nameof(config.PartyListSelf))
            .AddCheckbox(Strings.ApplyToPartyMembers, nameof(config.PartyListMembers))
            .AddCheckbox(Strings.ChangeHP, nameof(config.PartyListHpEnabled))
            .AddCheckbox(Strings.ChangeMP, nameof(config.PartyListMpEnabled))
            .AddCheckbox(Strings.ChangeGP, nameof(config.PartyListGpEnabled))
            .AddCheckbox(Strings.ChangeCP, nameof(config.PartyListCpEnabled));

        configWindow.AddCategory(Strings.ParameterWidget)
            .AddCheckbox(Strings.ShowOnParameterWidget, nameof(config.ParameterWidgetEnabled))
            .AddIndent()
            .AddCheckbox(Strings.ChangeHP, nameof(config.ParameterHpEnabled))
            .AddCheckbox(Strings.ChangeMP, nameof(config.ParameterMpEnabled))
            .AddCheckbox(Strings.ChangeGP, nameof(config.ParameterGpEnabled))
            .AddCheckbox(Strings.ChangeCP, nameof(config.ParameterCpEnabled));

        configWindow.AddCategory(Strings.Extra)
            .AddCheckbox(Strings.DeadOrAliveMode, nameof(config.DeadOrAliveModeEnabled));

        configWindow.AddCategory(Strings.PercentageSign)
            .AddCheckbox(Strings.ShowPercentageSign, nameof(config.PercentageSignEnabled));

        configWindow.AddCategory(Strings.PercentageFormat)
            .AddIntSlider(Strings.DecimalPlaces, 0, 2, nameof(config.DecimalPlaces))
            .AddCheckbox(Strings.ShowDecimalsOnlyBelow100, nameof(config.ShowDecimalsBelowHundredOnly));
    ...
    private void ModifyPartyListHp(PartyListHudData hudData, bool revertToDefault) {
        ...
        if (hudData.IsSelf() && config.PartyListSelf || !hudData.IsSelf() && !revertToDefault) {
            if (config.DeadOrAliveModeEnabled) {
                hpGaugeTextNode->SetText(health.Current > 0 ? Strings.Alive : Strings.Dead);
            }
            else {
